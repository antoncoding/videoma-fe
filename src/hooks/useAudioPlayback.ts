...
export function useAudioPlayback() {
  ...
  const playAudio = async (text: string, language: string, uniqueId?: number) => {
    ...
    const idWithVoice = `${uniqueId}-${voice}`.hashCode();

    setAudioLoading(true);
    try {
      let audioId = uniqueId ? getFromCache(idWithVoice) : null;
      ...
      if (!audioId) {
        ...
        if (uniqueId) {
          addToCache(idWithVoice, audio_id);
        }
      ...
      if (!audioResponse.ok) {
        if (audioResponse.status === 404) {
          if (uniqueId) addToCache(idWithVoice, 0);
